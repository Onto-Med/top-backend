...
public class DocumentQueryConfigMap {
        ...
        new HashMap<>() {
          {
            if (config.getConnection() != null) {
              put(SPRING_ES_URI_PROP, config.getConnection().getUrl() + ":" + config.getConnection().getPort());
            } else {
              put(SPRING_ES_URI_PROP, DEFAULT_ES_URL + ":" + DEFAULT_ES_PORT);
            }
            if (config.getIndex() != null) {
              // ToDo: look into this - provide list here possible, too?
              put(SPRING_ES_INDEX_PROP, Arrays.stream(config.getIndex()).findFirst().orElse(DEFAULT_ES_INDEX));
            } else {
  ...
  private TextAdapterConfig defaultConfig(Environment env) {
    ...
    elastic_connection.setUrl(DEFAULT_ES_URL);
    elastic_connection.setPort(DEFAULT_ES_PORT);
    if (env.getProperty(SPRING_ES_URI_PROP) != null) {
      ...
      elastic_connection.setUrl(url.getHost());
      elastic_connection.setPort(url.getPort() != -1 ? String.valueOf(url.getPort()) : DEFAULT_ES_PORT);
    }
    ...
    neo4j_connection.setUrl(DEFAULT_NEO4J_URL);
    neo4j_connection.setPort(DEFAULT_NEO4J_PORT);
    if (env.getProperty(SPRING_NEO4J_URI_PROP) != null) {
      ...
      neo4j_connection.setUrl(url.getHost());
      neo4j_connection.setPort(url.getPort() != -1 ? String.valueOf(url.getPort()) : DEFAULT_NEO4J_PORT);
    }
